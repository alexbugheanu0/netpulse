"""Job: shut down an interface (write operation — requires prior approval)."""
...
def run(device: Device, interface: str) -> JobResult:
    """
    Administratively shut down an interface.
    ...
    try:
        ...
        logger.info(
            f"shutdown_interface: {interface} shut down on {device.name}"
        )
        ...
        return JobResult(
            ...
            raw_output=output,
            parsed_data={"interface": interface, "state": "shutdown"},
        )

    except Exception as exc:
        logger.error(f"shutdown_interface failed on {device.name}: {exc}")
        return JobResult(
            success=False,
            device=device.name,
            intent="shutdown_interface",
